...
from transaction import get_coinbase_transaction, is_valid_address, processTransactions, Transaction, UnspentTxOut, TxIn, TxOut
...
class Blockchain(object):
    ...
    def __init__(self):
        ...
        self.unspenttx_outs = processTransactions([self.genesisTransaction], [], 0)
    ...
    def get_unspent_tx_outs(self):
        return self.unspenttx_outs

    def set_unspent_tx_outs(self, newUnspentTxOut):
        print('replacing unspenttx_outs with: %s' % newUnspentTxOut)
        self.unspenttx_outs = newUnspentTxOut

    @blocks.setter
    def blocks(self, blocks):
        self._blockchain = blocks

    # genesisTransaction = {'tx_ins': [{'signature': '', 'tx_out_id': '', 'tx_out_index': 0}],
    #     'tx_outs': [{
    ...
    genesisTransaction = Transaction([TxIn('', 0, '')], [TxOut(get_public_from_wallet(), 50)])
    ...
    def get_genesis_block(cls):
        ...
        return Block(0, "0", 1465154705, cls.genesisTransaction, 0, 0,
    ...
    def construct_next_block(self):
        coinbaseTx = get_coinbase_transaction(get_public_from_wallet(), self.blocks[-1].index + 1)
        blockData = [coinbaseTx] + get_transaction_pool()
        return self.generate_next_block(blockData)

    def construct_next_block_with_transaction(self, receiverAddress, amount):
        if not is_valid_address(receiverAddress):
        ...
        coinbaseTx = get_coinbase_transaction(get_public_from_wallet(), self.blocks[-1].index + 1)
        tx = create_transaction(receiverAddress, amount, get_private_from_wallet(), self.get_unspent_tx_outs(), get_transaction_pool())
        if not tx:
            return False
    ...
    def add_block(self, new_block):
        ...
        if self.is_valid_new_block(new_block, self.get_latest_block()):
            retVal = processTransactions(new_block.data, self.get_unspent_tx_outs(), new_block.index)
        if retVal is None:
            print('block is not valid in terms of transactions')
            return False
        else:
            self._blockchain.append(new_block)
            self.set_unspent_tx_outs(retVal)
